...
private[correlation] class MDCCorrelationIdObserver extends ContextObserver[CorrelationId] {
  import MDCCorrelationIdObserver._
  override def name: String = s"$MDCCorrelationIdKey.MDCObserver"
  override def put(value: CorrelationId): Unit = MDC.put(MDCCorrelationIdKey, value.id)
  override def removed(value: CorrelationId): Unit = MDC.remove(MDCCorrelationIdKey)
}
